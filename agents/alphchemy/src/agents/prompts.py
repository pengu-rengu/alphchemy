...
- `metrics` (array of metric names, non-empty): which metrics to compute and report in backtest results. Include `"total_entries"`, `"total_exits"`, and `"std_hold_time"` in generated experiments. Valid names: `"sharpe"`, `"excess_sharpe"`, `"max_drawdown"`, `"mean_hold_time"`, `"std_hold_time"`, `"total_entries"`, `"total_exits"`, `"signal_exits"`, `"stop_loss_exits"`, `"take_profit_exits"`, `"max_hold_exits"`
...
- `mean(<path>)`, `max(<path>)`, `min(<path>)`, and `std(<path>)` aggregate numeric values across every matching experiment and return one value. `min` and `max` include the first winning experiment id; `mean` and `std` do not include an id. Booleans are 0/1, `std` is population standard deviation, and selection wrappers cannot be nested
...
- `id` cannot be selected, filtered, or sorted; each window-selected value and each `min` or `max` aggregate is annotated with its experiment id in parentheses, e.g. `0.42 (100)`. ISO-8601 timestamp values are shown as `Jan 2 2026 12:00`
...
- `sort_asc: <path>` and `sort_desc: <path>` are optional and mutually exclusive. Sort paths must resolve to finite numbers or timestamps. Sorting happens after filtering and before selection windows; missing and non-finite values are skipped. Without a sort field, results remain ordered by `last_updated` descending
