...
pub mod tests {
    ...
    mod value_at_tests {
        ...
        #[hegel::test]
        fn test_value_at_min(tc: TestCase) {
            let range = tc.draw(gen_threshold_range());
            let n_thresholds = tc.draw(gen_usize_with_max(20)) + 2;
            let value = range.value_at(0, n_thresholds);
            assert_relative_eq!(value, range.min, epsilon = 1e-5);
        }

        #[hegel::test]
        fn test_value_at_max(tc: TestCase) {
            let range = tc.draw(gen_threshold_range());
            let n_thresholds = tc.draw(gen_usize_with_max(20)) + 2;
            let value = range.value_at(n_thresholds - 1, n_thresholds);
            assert_relative_eq!(value, range.max, epsilon = 1e-5);
        }

        #[hegel::test]
        fn test_value_at_zero_thresholds(tc: TestCase) {
            let range = tc.draw(gen_threshold_range());
            let value = range.value_at(3, 0);
            assert_relative_eq!(value, range.min, epsilon = 1e-5);
        }

        #[hegel::test]
        fn test_value_at_one_threshold(tc: TestCase) {
            let range = tc.draw(gen_threshold_range());
            let value = range.value_at(3, 1);
            assert_relative_eq!(value, range.min, epsilon = 1e-5);
        }

        #[hegel::test]
        fn test_value_at_interior(tc: TestCase) {
            let range = tc.draw(gen_threshold_range());
            let n_thresholds = tc.draw(gen_usize_with_max(20)) + 2;
            let value = range.value_at(1, n_thresholds);
            assert!(value >= range.min);
            assert!(value <= range.max);
        }
    }
