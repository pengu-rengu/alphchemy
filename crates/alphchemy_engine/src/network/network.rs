...
pub mod tests {
    ...
    mod feats_penalty_from_counts_tests {
        use super::*;
        ...
        fn test_feats_penalty_from_counts(tc: TestCase) {
            ...
            let penalty =
                feats_penalty_from_counts(n_used, n_used + n_unused, used_penalty, unused_penalty);
    ...
    mod node_ptr_tests {
        ...
        #[hegel::test]
        fn test_node_ptr_overflow(tc: TestCase) {
            let len = tc.draw(gen_usize_with_min(1));
            let ptr_from_start = tc.draw(gen_node_ptr(len, Some(Anchor::FromStart), true));
            let ptr_from_end = tc.draw(gen_node_ptr(len, Some(Anchor::FromEnd), true));

            let from_start_idx = ptr_from_start.abs_idx(len);
            let from_end_idx = ptr_from_end.abs_idx(len);

            assert_eq!(from_start_idx, None);
            assert_eq!(from_end_idx, None);
        }
    }
}
