...
trait BacktestDeps {
    ...
    fn sharpe(&self, values: &[f64]) -> f64 {
        _sharpe(&BacktestDepsImpl, values).unwrap()
    }
...
impl BacktestState {
    ...
    fn _risk_exits_update<D>(&mut self, deps: &D, stop_loss: f64, take_profit: f64, max_hold_time: usize, idx: usize) where D: BacktestDeps {
        ...
        let conds = deps.exit_conds(lot_ref, stop_loss, take_profit, max_hold_time, self.close_prices[idx], idx);
        if !conds.any() {
            return;
        }
        ...
        self.take_profit_exits += usize::from(conds.take_profit);
        self.stop_loss_exits += usize::from(conds.stop_loss);
...
fn _sharpe<D>(deps: &D, values: &[f64]) -> Result<f64, String> where D: BacktestDeps {
    if values.len() < 3 {
...
mod tests {
    ...
    fn gen_backtest_state(tc: TestCase) -> BacktestState {
        ...
        state.max_hold_exits = tc.draw(gen_usize());
        state.signal_exits = tc.draw(gen_usize());
