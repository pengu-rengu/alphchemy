...
#[test]
fn test_single_lot_blocks_concurrent_entry_same_schema() {
...
#[test]
fn test_single_lot_blocks_other_schema_when_open() {
    let close_prices = vec![100.0, 100.0];
    let signals = vec![
        make_net_signal(
            &[("entry_0", true), ("entry_1", true)],
            &[("exit_0", false), ("exit_1", false)]
        ),
        make_net_signal(
            &[("entry_0", false), ("entry_1", false)],
            &[("exit_0", false), ("exit_1", false)]
        )
    ];
    ...
    assert_eq!(results.final_state.entries, 1);
    let lot = results.final_state.lot.as_ref().unwrap();
    assert_eq!(lot.schema_id, "entry_0");
}

#[test]
fn test_slot_frees_after_exit() {
    ...
    let lot = results.final_state.lot.as_ref().unwrap();
    assert_eq!(lot.schema_id, "entry_1");
}
...
fn test_exit_signal_uses_schema_id() {
    ...
    let signals = vec![
        make_net_signal(
            &[("entry_0", true), ("entry_1", false)],
            &[("exit_0", false), ("exit_1", false)]
        ),
        make_net_signal(
            &[("entry_0", false), ("entry_1", false)],
            &[("exit_0", false), ("exit_1", true)]
        ),
        make_net_signal(
            &[("entry_0", false), ("entry_1", false)],
            &[("exit_0", true), ("exit_1", false)]
        )
    ];
    ...
    assert_eq!(results.final_state.signal_exits, 1);
    assert!(results.final_state.lot.is_none());
}
